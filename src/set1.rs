...
pub fn break_repeating_key_xor() -> Vec<u8> {
    let file = File::open("inputs/set1/challenge6.txt").expect("could not open challenge 6 input");
    let reader = BufReader::new(file);
...
pub fn aes128_ecb_mode(ciphertext: &[u8], key: &[u8]) -> Vec<u8> {
    let mut buf = ciphertext.to_vec();
    let decryptor = Decryptor::<Aes128>::new(key.into());
    decryptor
        .decrypt_padded_mut::<ecb::cipher::block_padding::Pkcs7>(&mut buf)
        .unwrap()
        .to_vec()
}
...
mod tests {
    ...
    fn test_part7() {
        let file =
            File::open("inputs/set1/challenge7.txt").expect("could not open challenge 7 input");
        let reader = BufReader::new(file);
